...
function Navbar({data}) {
  ...
  const handleKeyDown = async (e) => {    
    if (e.key === "Enter") {
      const data = await getQueryPhotos(query);
      setPhotos(data);
    }
  };
  ...
  return (
    <div className="p-5">
      <div className="flex justify-between items-center h-[52px] w-[95%] mx-auto">
        ...
        <div className="bg-gray-200 w-[940px] rounded-full p-[7px] flex items-center justify-center">
          <span className="text-xl p-3">
            <CgSearch />
          </span>
          <input
            type="search"
            className="w-[880px] h-[38px] outline-none text-black"
            placeholder="Search"
            value={query}
